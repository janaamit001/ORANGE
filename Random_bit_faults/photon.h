...
u8* fault_on_AddRC(u8* State, int round, u8 fault, u8 pos)
{
    int i;
    for(i = 0; i < 8; i++)
        State[4*i] ^= RC[i][round];
...
/*void invMixColumn(unsigned char state[8][8])
{
	int i, j, k;
	unsigned char tmp[8];
	for(j = 0; j < 8; j++){
...
u8* fault_on_PHOTON_256_Permutation(u8* State, u8 fault, u8 pos)
{
    int i;
    ...
    for(i = 0; i < 12; i++)
    {
		...
		State = fault_on_AddRC(State, i, fault, pos);
		State = SubCell(State);
		State = ShiftRow(State);
		State = MixColumn(State);
	}

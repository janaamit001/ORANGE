...
int generate_test_vectors()
{
	...
	unsigned char fault, fault1, diff, pos, temp = -1;
	...
	for( pos = 0; pos < 64; ++pos ) {	
		...
		for( int i1 = 0; i1 < 11; ++i1 ) {
			...
			//fault = ((rand()%256) & 0xf0);
			printf("At i1 = %d  ", token1[i1]);
			if((pos%8)%2 == 0)
				st_diff[ 4*(pos/8) + (pos%8)/2 ] = (token1[i1] << 4);
			else
				st_diff[ 4*(pos/8) + (pos%8)/2 ] = token1[i1];
			//st_diff[0] = (i1 << 4);
			...
			if (faulty_crypto_aead_decrypt(msg2, &mlen2, NULL, ct1, clen, ad, adlen, nonce, key, fault, pos) == 0) {
				...
				if((pos%8)%2 == 0) {
					fault1 = rand()%4;
					fault1 = (token[fault1] << 4);
					while((fault1 == fault) || (fault1 == temp)) {
